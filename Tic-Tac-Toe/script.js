//Get the board, record, select, and message html elements
...
function init() {
  ...
  for (let i = 0; i < gridSpaces.length; i++) {
    gridSpaces[i].addEventListener("click", () => {
      ...
      //AI move
      if (inPlay) {
        if (difficulty != 0) makeAiMove();
        else player = player == "O" ? "X" : "O";
      }
    });
  }
}
...
function checkDiagonal(index, x, y, board) {
  if (x == y) return getGridSpace(index, index, board);
  else return null;
}
...
function minimax(board, depth, maximizingPlayer) {
  ...
  if (depth == 0 || isTerminating(board) || score != 0) return score;
  if (maximizingPlayer) {
    let value = -10;
  ...
  for (let i = 0; i < 2; i++) {
    for (let j = 0; j < 3; j++) {
      if (getWin(j, j, currentPlayer, board).length == 3) {
        if (currentPlayer == "O") return 10 - (difficulty - depth);
        else return -10 + (difficulty - depth);
      }
    }
    currentPlayer = "X";
  }
...
function getSpaceValue(x, y, board) {
  if (x == null) return;
  else if (typeof x === "object") return x.firstChild.textContent;
  else if (y == undefined) return gridSpaces[x].firstChild.textContent;
  else return gridSpaces[y * gridWidth + x].firstChild.textContent;
}
...
function getGridSpace(x, y, board) {
  if (board != undefined) return board[y * gridWidth + x];
  else return gridSpaces[y * gridWidth + x];
}
